...
const UsersNav = (props) => {
    if(props.isAuthenticated) {
        ...
        if (Array.from(props.user.tasks).includes('createNewSeller')) list.push(<li key={'createNewSeller'}>
            <Link className="usersNavigationContainerLink" to="/newSeller">Create new seller</Link></li>);
        if (Array.from(props.user.tasks).includes('createSellerCard')) list.push(<li key={'createSellerCard'}>
            <Link className="usersNavigationContainerLink" to="/sellerCards">Sellers cards</Link></li>);
        if (Array.from(props.user.tasks).includes('getCall')) list.push(<li key={'getCall'}>
            <Link className="usersNavigationContainerLink" to="/calls">Calls</Link></li>);
        if (Array.from(props.user.tasks).includes('inspection')) list.push(<li key={'inspection'}>
            <Link className="usersNavigationContainerLink" to="/inspection">inspection</Link></li>);
        if (props.user.role === 'manager') list.push(<li key={'admin'}>
